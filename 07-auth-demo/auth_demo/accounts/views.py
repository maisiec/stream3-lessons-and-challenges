...
def subscriptions_webhook(request):
    ...
    # Verify the even by fetching it from Stripe
    try:
        ...
        paid = User.objects.get(stripe_id=cust)
        if user and paid:
            user.subscription_end = arrow.now().replace(week=+4).datetime  # 4 weeks from now
            user.save()
